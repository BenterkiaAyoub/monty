 ...
 * Return: Returns 0 if the opcode corresponds to a stack,
...
int parseLn(char *buffer, int lenn, int format)
{
	...
	funFind(opcode, vl, lenn, format);
	return (format);
}

/**
 * funFind - Finds the appropriate function for the opcode.
 * @opcode: Opcode.
 * @vl: Argument of the opcode.
 * @format: Storage format. If 0, nodes will be entered as a stack.
 * If 1, nodes will be entered as a queue.
 * @ln: Line number.
 * Return: void
...
void funFind(char *opcode, char *vl, int ln, int format)
{
	int ij;
	int flg;
	...
	for (flg = 1, ij = 0; func_list[ij].opcode != NULL; ij++)
	{
		if (strcmp(opcode, func_list[ij].opcode) == 0)
		{
			call_fun(func_list[ij].f, opcode, vl, ln, format);
			flg = 0;
		}
	}
	if (flg == 1)
